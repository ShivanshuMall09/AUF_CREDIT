...
AUF_CREDIT_ProcessV.17.1.23.16.17.py:19-06-2025[Adding PP for 653023 and logo 924 for pp generation] 
AUF_CREDIT_ProcessV.17.1.23.16.16.py:06-06-2025[rejecting the special character check from the fome bin which is given in the Exclude_Bin_logo_special_character file]
AUF_CREDIT_ProcessV.17.1.23.16.15.py:04-06-2025[Generating Batchcard extra file reconcilation and material return file]
AUF_CREDIT_ProcessV.17.1.23.16.14.py:21-05-2025[Adding new code of Embo checker function]
AUF_CREDIT_ProcessV.17.1.23.16.13.py:19-05-2025[Changing the masking in scanning file last 4 to last 6 digit in scanning card file
AUF_CREDIT_ProcessV.17.1.23.16.12.py:14-05-2025[1. changing deletion log file on the request of aditya for vdate change
                                                2.-There is change for discontinuation of Altura Celeb Female product.Now if plastic_id: 0000000003 then product will be Altura.And if blank plastic_id also then product will be Altura or Altura_Rupay, there is no celeb_male and celeb_female product, it should show Altura_Rupay only It all should be reflected in allocation and mailer file.]
AUF_CREDIT_ProcessV.17.1.23.16.11.py:09-05-2025[resolving error ]
...
for w in (FutureWarning, SyntaxWarning):
    warnings.simplefilter(action="ignore", category=w)


try:
    print("AUF_CREDIT_ProcessV.17.1.23.16.17.exe running!!!!")
...
#=============================================Excel-Prcocessing starts=============================================           
        ...
        def label_sort(line):
            ...
            return parts[13],parts[6],parts[4]   
...
#=============================================File-Prcocessing starts=============================================   
        ...
        if len(file_name)==0:
           ...
           #=============================================  
            ...
            with open('config\Exclude_Bin_logo_special_character.csv', 'r') as IXIGOin:
                contents = IXIGOin.readlines()
                # binlogos = contents.split(',')
                for binlogo in contents:
                    Bin_Logo_check_list.append(binlogo.replace('\n',''))

            # print(Bin_Logo_check_list)
                ...
                for fname_acc in merge_filename_acc:
                    with open(fname_acc) as infile_acc:
                        for line_acc in infile_acc:
                            ...
                            Bin_Logo_check = c_bin_f_acc+logo_acc
                            _4TH_LINE_NAME = line_acc[94:130]
                            name_check = ''
                            name_check_4line = ''
                            name_check = contains_special_characters(print_embo_name)

                            if Bin_Logo_check not in Bin_Logo_check_list:
                                # input()
                                name_check_4line = contains_special_characters(_4TH_LINE_NAME)
                            else:
                                name_check_4line = True


                            if name_check == False or name_check_4line == False:
                                ...
                                last_six_digit_mask_pan_no = cardnumber_acc[0:6] +'XXXX'+cardnumber_acc[10:16]
                                ...
                                if logo_acc != '306':
                                    if ((c_bin_acc == '465523_1' and card_holder_type_acc=='1') or (c_bin_acc == '457036')  or (c_bin_acc == '653024' and logo_acc == '910') or (c_bin_acc == '653023' and logo_acc == '924') or  (c_bin_acc == '653023' and  logo_acc == '912')):#and logo_acc == '402'):
                                    ...
                                    if ((c_bin_acc == '465523_1' and card_holder_type_acc=='1') or (c_bin_acc == '457036') or (c_bin_acc == '653024' and logo_acc == '910') or (c_bin_acc == '653023' and logo_acc == '924') or (c_bin_acc == '653023' and  logo_acc == '912') ):#and logo_acc == '402'):
                                        if (card_action_code == '1' or card_action_code == '2' or card_action_code == '7' or card_action_code == 'A'):
                                            ...
                                            elif ((c_bin_acc == '457036') or (c_bin_acc == '653024' and logo_acc == '910') or (c_bin_acc == '653023' and logo_acc == '924') or (c_bin_acc == '653023' and  logo_acc == '912')):#Zenith cards
                                ...
                                scanning_tapping_primary = '|'+account_number_acc+'|'+last_six_digit_mask_pan_no+'|'+print_card_holder_name_acc+'|||||'+pp_cardnumber_acc+'||||'+ref_number+'|' +str(awb_number)+'|'+courier+'|'+routing_code+'|'+card_action+'|1|'+totol_addon_card_count+'|'+totol_card_count+'|'+c_bin_f_acc+'|'+logo_acc+'|'+allocation_gender_code+'|'+varient+'|'+mailer_address1_acc+'|'+mailer_address2_acc+'|'+mailer_address3_acc+'|' +mailer_address4_acc+'|'+mailer_city_acc+'|'+mailer_state_code_acc+'|'+mailer_postal_code_acc+'|' +mailer_mob_acc+'|'+customer_id_acc+'|'+pp_count+'|'+joining_fees +'|'+annual_fees+'|'+str(jdtodatestd(date_acc)+'|'+retail_card_type+'|'+plastic_id+'|'+product_code)
                                ...
                                scanning_tapping_addon = '|'+account_number_acc+'||'+card_holder_name_acc.rstrip()+'|'+last_six_digit_mask_pan_no+'|||||'+pp_cardnumber_acc+'|||'+ref_number+'|'+str(awb_number)+'|'+courier+'|'+routing_code+'|'+card_action+'-Additional Card'+'|0|'+totol_addon_card_count+'|'+totol_card_count+'|'+c_bin_f_acc+'|'+logo_acc+'|'+allocation_gender_code+'|'+varient+'|'+mailer_address1_acc +'|'+mailer_address2_acc+'|'+mailer_address3_acc+'|'+mailer_address4_acc+'|'+mailer_city_acc+'|'+mailer_state_code_acc+'|' +mailer_postal_code_acc+'|'+mailer_mob_acc+'|'+customer_id_acc+'|' +pp_count+'|'+joining_fees+'|'+annual_fees +    '|'+str(jdtodatestd(date_acc)+'|'+retail_card_type+'|'+plastic_id+'|'+product_code)
                                ...
                                if card_holder_type_acc == '1':
                                    ...
                                    with open('xxAUF_Credit_Scanning_card_tapping.txt', 'a') as ff_file_Scan:
                                        ff_file_Scan.write(scanning_tapping_primary+'\n')
                                    ff_file_Scan.close()
                                    ...
                                    with open('xxAUF_Credit_Scanning_card_tapping.txt', 'a') as ff_file_Scan:
                                        ff_file_Scan.write(scanning_tapping_addon+'\n')
                                    ff_file_Scan.close()
    ...
    # =============================================================================
            ...
            with open('xxAUF_Credit_Scanning_card_tapping.txt', 'r') as file_alloc:
            ...
            for file in files:
                ...
                with open(file,'r') as infile,open('xAUF_CREDIT_SCANNING_CARD_FILE.txt','a') as outfile,open('xAUF_CREDIT_LABEL_FILE.txt','a') as lblfile:
                    ...
                    lblfile.write('|'+Customer_Name+'|'+Refno+'|'+AWB_No+'|'+Courier+'|'+Courier_Code+'|'+Card_Action+'|'+Primary_Card_count+'|'+str(addon_count)+'|'+str(size)+'|'+Bin+'|'+Logo+'|'+Gender_Code+'|'+Varient+'|'+Address_Line_1+'|'+Address_Line_2+'|'+Address_Line_3+'|'+Address_Line_4+'|'+City+'|'+State_Code+'|'+Postal_Code+'|'+Mobile_Number+'|'+Customer_Name+'|'+''+'|'+product_code+'\n')        
            ...
            try:
                ...
                print(f"Error processing IndiaPost: {e}")
            ...
            try:
                ...
                print(f"Error processing Delhivery: {e}")
            ...
            try:
                ...
                print(f"Error processing Bluedart: {e}")
            ...
            try:
                ...
                print(f"Error protecting Excel files: {e}")
            ...
            try:
                ...
                print(f"Error in FF MIS Merge: {e}")    
            ...
            try:
                ...
                print(f"Error in Data Allocation Report: {e}")
            ...
            with open('xAUF_CREDIT_LABEL_FILE.txt', 'r') as infile, open('AUF_CREDIT_LABEL_FILE.csv', 'a') as scan_file:
                previous_card_action, previous_varient = '', ''
                contents = infile.readlines()
                sorted_lines = sorted(contents, key=label_sort)
                Srn = 0

                scan_file.write('Sr. No|Customer Name|Ref no.|AWB. No.|Courier|Courier Code|Card Action|Primary Count|Add-on Count|Total Cards|Bin|Logo|Gender Code|Varient|Address|Mobile Number|Embo_Name|Card Type|Product_code\n')

                for ln in sorted_lines:
                    parts = ln.strip().split('|')
                    merged_address = ' '.join([parts[14], parts[15], parts[16], parts[17], parts[18], parts[19], parts[20]]).strip()

                    card_action = parts[6].strip()
                    varient = parts[13].strip()

                    if card_action != previous_card_action or varient != previous_varient:
                        Srn = 0
                    Srn += 1

                    new_parts = parts[1:14] + [merged_address] + parts[21:25]
                    new_parts = list(map(str, new_parts))

                    scan_file.write(str(Srn).zfill(4) + '|' + '|'.join(new_parts) + '\n')

                    previous_card_action = card_action
                    previous_varient = varient




            df = pd.read_csv('AUF_CREDIT_LABEL_FILE.csv',encoding= 'unicode_escape', sep='|', dtype=object)
            df.to_excel('AUF_CREDIT_LABEL_FILE_'+str(ptime[0:10]) +'--'+str(s_no) +'+'+str(total_pp_count)+'.xlsx', 'Sheet1', index=False)
            os.remove('AUF_CREDIT_LABEL_FILE.csv')
            os.remove('xAUF_CREDIT_LABEL_FILE.txt')
            ...
            with open('xxAUF_Credit_Scanning_File.txt', "r") as ff, open('AUF_Credit_Scanning_File.txt', "w") as foutfile:
                ...
                for fi in contents:
                    ...
                    if fi:
                        s_no+=1
                        foutfile.write(str(s_no).zfill(4)+fi)
            ff.close()
            foutfile.close()
            ...
            excel_files = glob.glob(os.path.join(current_directory, '*FF*.xlsx'))+glob.glob(os.path.join(current_directory, '*label*.xlsx')) + glob.glob(os.path.join(current_directory, '*PP*.xlsx')) + glob.glob(os.path.join(current_directory, '*Scan*.xlsx'))+glob.glob(os.path.join(current_directory, '*Courier_Connection*.xlsx'))
