...
@author: rajeev.jadaun
      AUF_CREDIT_ProcessV.15.5.2.py:12-04-2023[Configurable product and varient , batch cards]  
...
# =============================================================================
# =============================================================================
                            loader = importlib.machinery.SourceFileLoader('product_varient', 'config/product_varient' )
                            spec = importlib.util.spec_from_loader( 'product_varient', loader )
                            product_varient = importlib.util.module_from_spec( spec )
                            loader.exec_module( product_varient )
                            var_prod = product_varient.return_prod_var(str(c_bin_acc),str(logo_acc),str(plastic_id),str(gender_code_acc))
...
#                     dg_color = 'Not Found'
# =============================================================================
                loader = importlib.machinery.SourceFileLoader('batchcard', 'config/batchcard' )
                spec = importlib.util.spec_from_loader( 'batchcard', loader )
                batchcard = importlib.util.module_from_spec( spec )
                loader.exec_module( batchcard )
                batch_data = batchcard.return_batchcard(str(key))
                batch_data = batch_data.split('|')
                art_work = batch_data[0]
                job_setup = batch_data[1]
                dg_color = batch_data[2]                            
