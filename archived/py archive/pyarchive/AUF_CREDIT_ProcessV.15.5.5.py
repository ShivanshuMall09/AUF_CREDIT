...
AUF_CREDIT_ProcessV.15.5.5.py:18-25-2023[logic for anual ,joining and monthly fees.]  
AUF_CREDIT_ProcessV.15.5.4.py:25-04-2023[Configurable product and varient , batch cards]  
...
#=============================================Fees-Logic=============================================                
        def fees(logo,pct,creditlimit):   
            ...
            with open("./config/Fees_console.csv","r") as data_read:
                ...
                if joining_fees.__contains__('%'):
                    joining_fees_percentage = int(joining_fees.replace('%',''))
                    joining_fees = round((joining_fees_percentage * int(creditlimit) )//100)
                    if joining_fees<1000:
                        joining_fees = '1000'
                    else:
                        joining_fees = str(joining_fees)
                
                if annual_fees.__contains__('%'):
                    annual_fees_percentage = int(annual_fees.replace('%',''))
                    annual_fees = round((annual_fees_percentage * int(creditlimit) )//100)
                    if annual_fees<1000:
                        annual_fees = '1000'
                    else:
                        annual_fees = str(annual_fees)
                ...
                for fname_acc in merge_filename_acc:
                    with open(fname_acc) as infile_acc:
                        for line_acc in infile_acc:
                            ...
                            cr_card_limit_1 = line_acc[167:182]
                            cr_card_limit_1 = cr_card_limit_1.lstrip('0')
                            cr_card_limit = format_currency(cr_card_limit_1, '', locale='en_IN')
...
# =============================================================================                            
                            ...
                            joining_fees = fees(logo_acc,pct_id,cr_card_limit_1)[0]
                            annual_fees = fees(logo_acc,pct_id,cr_card_limit_1)[1]
...
# =============================================================================
            ...
            for file in ff_filename:
                try:
                    ...
                    print("Error while converting file : "+file+ "  "+ str(e))
