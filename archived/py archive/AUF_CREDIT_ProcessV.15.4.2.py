...
@author: rajeev.jadaun
      AUF_CREDIT_ProcessV.15.4.2.py:09-02-2023[Monthly fees from PCT_ID console]
      ...
      #-----------------------PP DUP CHECK------------------------------------------------------------   
            ...
            with open("./config/Fees_console.csv","r") as data_read:
                ...
                for line in contents:
                    ...
                    if (logo_id == logo) and (pct_id == pct):
                        joining_fees_1 = line[2].rstrip()
                        annual_fees_1 = line[3].rstrip()
                        # monthly_fees_1 = line[4].rstrip()
                        break
                    
                return(joining_fees_1,annual_fees_1)
        ...
        def ip_weight(cardaction):
            ...
            try:   
                ...
                print('Error in AU_Credit_Variant_Weight_Table.csv')
            
                            
        
        def awb_assign(courier_2):
            ...
            for fname_acc in merge_filename_acc:
                with open(fname_acc) as infile_acc:
                    for line_acc in infile_acc:
                        ...
                        # monthly_fees = fees(logo_acc,pct_id)[2]
                        ...
                        allocation_data_addon = '|'+account_number_acc+'||'+card_holder_name_acc.rstrip()+'|'+mask_pan_acc+'|||||'+pp_cardnumber_acc+'|||'+ref_number+'|'+str(awb_number)+'|'+courier+'|'+routing_code+'|'+card_action+'-Additional Card'+'|0|1|1|'+c_bin_f_acc+'|'+logo_acc+'|'+gender_code_acc+'|'+varient+'|'+mailer_address1_acc +'|'+mailer_address2_acc+'|'+mailer_address3_acc+'|'+mailer_address4_acc+'|'+mailer_city_acc+'|'+mailer_state_code_acc+'|' +mailer_postal_code_acc+'|'+mailer_mob_acc+'|'+customer_id_acc+'|' +pp_count+'|'+joining_fees+'|'+annual_fees +    '|'+str(jdtodatestd(date_acc))
