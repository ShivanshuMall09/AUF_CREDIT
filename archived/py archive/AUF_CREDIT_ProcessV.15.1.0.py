        ...
        with open(fname_acc) as infile_acc:
            for line_acc in infile_acc:
                ...
                for i in enumerate(line_acc):
                    ...
                    mailer_name1_acc = (line_acc[1518:1558].encode("ascii", "ignore")).rstrip()
                    mailer_name2_acc = str((line_acc[1558:1598].encode("ascii", "ignore")).rstrip())
                    mailer_name3_acc = str((line_acc[1598:1638].encode("ascii", "ignore")).rstrip())
                    mailer_name_acc = str(mailer_name1_acc)+' '+str(mailer_name2_acc)+' '+str(mailer_name3_acc)
                    mailer_address1_acc = str((line_acc[1638:1678].encode("ascii", "ignore")).rstrip())
                    mailer_address2_acc = str((line_acc[1678:1718].encode("ascii", "ignore")).rstrip())
                    mailer_address3_acc = str((line_acc[1718:1758].encode("ascii", "ignore")).rstrip())
                    mailer_address4_acc = str((line_acc[1878:1918].encode("ascii", "ignore")).rstrip())
                    mailer_city_acc = str((line_acc[1758:1788].encode("ascii", "ignore")).rstrip())
                    mailer_state_code_acc = str((line_acc[1838:1868].encode("ascii", "ignore")).rstrip())
                    mailer_postal_code_acc = str((line_acc[2016:2026].encode("ascii", "ignore")).rstrip())
                    mailer_mob_acc = str((line_acc[3143:3163].encode("ascii", "ignore")).rstrip())
                    suffix = ""
...
def pp_excel_convertor(pp_name_ff):
    ...
    if Path(pp_name_ff).is_file():
        ...
        ff.close()
        foutfile.close()
        df = pd.read_csv(pp_name_ff[2:], sep='|', dtype=object)
