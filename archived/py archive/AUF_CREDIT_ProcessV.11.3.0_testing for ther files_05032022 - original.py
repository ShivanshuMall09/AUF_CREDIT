...
#-----------------------------------------------------------------------------------------
                ...
                ps1 = account_number_acc+'|'+card_type+'|'+account_number_acc+'|'+mask_pan_acc+'|'+mailer_name_acc+'|'+card_action+'|'+c_bin_f_acc+'|'+logo_acc+'|'+gender_code_acc+'|'+varient+'|'+awb_number+'|'+routing_code+'|'+print_card_holder_name_acc+'|'+joining_fees+'|'+annual_fees+'|'+str(jdtodatestd(date_acc))
...
if my_file10.is_file():
    with open('xxAUC_FF_MIS_test_B466505_L101_G1_Altura_C_Female.txt',"r") as ff, open('AUC_FF_MIS_test_B466505_L101_G1_Altura_C_Female.txt',"w") as foutfile:
        foutfile.write("Sr. No|Account Number|Primary Card number|Customer Name|Add-on Card 1|Add-on Card 2|Add-on Card 3|Add-on Card 4|PP  Card No_Primary|PP  Card No_Addon1|PP  Card No_Addon2|PP  Card No_Addon3|Ref no.|AWB. No.|Courier|Courier Code|Card Action|Primary Count|Add-on Count|Total Cards|Bin|Logo|Gender Code|Varient|Address Line 1|Address Line 2|Address Line 3|Address Line 4|City|State Code|Postal Code|Mobile Number|Cust Unique ID|Credit Limit|Statement Date |Extention|Embo_Name|Joining Fees|Annual Fees|Card Issuance Date"+'\n')
        contents = ff.readlines()
        # sorting using our custom logic
        contents.sort(key=my_sort_ff)
        for fi in contents:
            ...
            if fi:
                foutfile.write(str(s_no_10)+fi)
    df = pd.read_csv('AUC_FF_MIS_test_B466505_L101_G1_Altura_C_Female.txt', sep='|', dtype=object)
    df.to_excel('AUC_FF_MIS_test_B466505_L101_G1_Altura_C_Female'+str(s_no_10)+'.xlsx', 'Sheet1',index=False)
