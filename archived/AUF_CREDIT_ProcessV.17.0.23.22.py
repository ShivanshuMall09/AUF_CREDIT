...
AUF_CREDIT_ProcessV.17.1.23.22.py:27-02-2025[version name in bacthcards]
AUF_CREDIT_ProcessV.17.1.23.21.py:26-02-2025[removing chip name column from the batchcard]
AUF_CREDIT_ProcessV.17.1.23.19.py:14-01-2025[.. replacing it occur due to padding fault]
AUF_CREDIT_ProcessV.17.1.23.16.py:07-01-2025[Adding product matrix]
...
try:
    print("AUF_CREDIT_ProcessV.17.1.23.17.exe running!!!!")
        ...
        def product_matrix(Product_name,card_type):
            ...
            with open('config/AUFC_PRODUCT_MATRIX.txt','r') as pd_file:
                ...
                if card_type == 'New Issue':
                    ...
                    card_type_code = 'DR'
                elif card_type == 'Reissue Card' or card_type == 'Emergency Card Requested' or card_type == 'Reissue Card with different card numbering scheme' or card_type == 'Card technology reissue':
                    card_type_code = 'LS'
...
#=============================================File-Prcocessing starts=============================================   
        ...
        if len(file_name)==0:
           ...
           #=============================================  
                ...
                for fname_acc in merge_filename_acc:
                    with open(fname_acc) as infile_acc:
                        for line_acc in infile_acc:
                            ...
                            # print(line_acc[54:80])
                            # print(print_card_holder_name_acc)
                            name_check = ''
                            name_check = contains_special_characters(print_card_holder_name_acc)
                            # print(print_card_holder_name_acc)
                            # if print_card_holder_name_acc.__contains__('.'):
                            #     print(print_card_holder_name_acc)
                            if name_check == False:
                                ...
                                # print(product+','+card_action+','+product_code)
                                ...
                                allocation_data_addon = '|'+account_number_acc+'||'+print_card_holder_name_acc.rstrip()+'|'+mask_pan_acc+'|||||'+pp_cardnumber_acc+'|||'+ref_number+'|'+str(awb_number)+'|'+courier+'|'+routing_code+'|'+card_action+'-Additional Card'+'|0|'+totol_addon_card_count+'|'+totol_card_count+'|'+c_bin_f_acc+'|'+logo_acc+'|'+gender_code_acc+'|'+varient+'|'+mailer_address1_acc +'|'+mailer_address2_acc+'|'+mailer_address3_acc+'|'+mailer_address4_acc+'|'+mailer_city_acc+'|'+mailer_state_code_acc+'|' +mailer_postal_code_acc+'|'+mailer_mob_acc+'|'+customer_id_acc+'|' +pp_count+'|'+joining_fees+'|'+annual_fees +'|'+str(jdtodatestd(date_acc)+'|'+retail_card_type+'|'+plastic_id)
                                if logo_acc != '306':
                    ...
                    for data in datas[1:]:
                        ...
                        if (bin_num == key[8:14] and job_setup_batch == job_setup and  art_work_batch == art_work or file_name_batch in key):
                            ...
                            with open('AUF_CREDIT_BATCHCARD_FILE.csv','a') as out_file:
                                ...
                                out_file.write(''+'|'+str(value)+'|'+key+'|'+job_setup_batch+'|'+art_work_batch+'|'+printing_front+'|'+ribbon_front+'|'+printing_back+'|'+ribbon_back+'|'+bin_num+'|'+logo_batch+'\n')
                            break
            ...
            # df.to_excel('AUFC_BATCHCARD_'+ptime[0:10]+'_'+str(sum(names.values()))+'.xlsx', 'Sheet1', index=False)       
            
            # input_file = glob.glob('AUFC_BATCHCARD_*.xlsx')
